      ...
      try {
        ...
        if (existingClass) {
          return res.status(StatusCodes.BAD_REQUEST).json({
            ...
            errors: [
              { message: ClassMsg.EXISTS(classData.number), path: 'number' },
            ],
          });
        }
    ...
    async getClasses(req = request, res = response, next) {
      try {
        ...
        if (
          ...
          (capacity && isNaN(parsedCapacity))
        ) {
          return res.status(StatusCodes.BAD_REQUEST).json({
            success: false,
            message: 'Invalid query parameters',
            errors: [
              { message: 'Invalid query parameters', path: 'query_parameters' },
            ],
          });
        }
    ...
    async updateClass(req = request, res = response, next) {
      try {
        ...
        const updatedData = _.omitBy(
          req.body,
          (value, key) => _.isNil(value) || value === '' || key === 'capacity'
        );
        ...
        if (updatedData.number) {
          const conflictingClass = await this.#model.findOne({
            where: { number: updatedData.number },
          });
