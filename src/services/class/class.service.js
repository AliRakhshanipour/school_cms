  ...
  class ClassService extends BaseService {
      ...
      try {
        ...
        if (!Number.isInteger(newCapacity) || newCapacity <= 0) {
          return res.status(StatusCodes.BAD_REQUEST).json({
            ...
            errors: [
              { message: ClassMsg.INVALID_CAPACITY(), path: 'new_capacity' },
            ],
          });
        }
      ...
      try {
        ...
        if (studentIds.length === 0 && nationalCodes.length === 0) {
          return res.status(StatusCodes.BAD_REQUEST).json({
            success: false,
            message:
              "Invalid input. At least one of 'studentIds' or 'nationalCodes' must be provided.",
            errors: [
              { message: 'No student identifiers provided', path: 'input' },
            ],
          });
        }
        ...
        const existingNationalCodes = new Set(
          allStudents.map((student) => student.national_code)
        );

        // Filter out non-existing students
        studentIds = studentIds.filter((id) => existingStudentIds.has(id));
        nationalCodes = nationalCodes.filter((code) =>
          existingNationalCodes.has(code)
        );
        ...
        if (studentIds.length === 0 && nationalCodes.length === 0) {
          return res.status(StatusCodes.BAD_REQUEST).json({
            ...
            errors: [
              {
                message:
                  'All provided student IDs or national codes are invalid.',
                path: 'students',
              },
            ],
          });
        ...
        const uniqueStudentIds = Array.from(
          new Set(allStudents.map((student) => student.id))
        );

        // Check if students are already enrolled in another class
        const studentsAlreadyEnrolled = await this.#checkEnrollment(
          uniqueStudentIds,
          classId
        );
      ...
      try {
        ...
        if (!classInstance) {
          return res.status(StatusCodes.NOT_FOUND).json({
            ...
            errors: [
              { message: ClassMsg.NOT_FOUND(classIdInt), path: 'classId' },
            ],
          });
        }
    ...
    async #checkEnrollment(studentIds, classId) {
      ...
      return enrolledStudents.filter(
        (student) => student.class && student.class.id !== classId
      );
    }
  }
